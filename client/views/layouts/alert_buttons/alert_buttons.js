Template.alert_buttons.events({
    
'click #helpButton': function(evt, tmpl){
    var documentVals = {
    ...
    while (1) {
        var cursor = HelpPostCollection.find({});
 //       var cursor = HelpPostCollection.find( {}, { _id: 1 } ).sort( { _id: -1 } ).limit(1);
 ...
 //       var seq = cursor.hasNext() ? cursor.next()._id + 1 : 1;
        ...
        var results = HelpPostCollection.insert(doc);
...
//        }
    ...
    HelpPostCollection.update({_id: doc._id},{$push:{posts:firstPost}});
};
}
});